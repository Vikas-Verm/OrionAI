...
function buildPlannerPrompt({
  ...
  return [
    ...
    `4. SLACK/TELEGRAM/WHATSAPP DMs: Use the person's name as channel/contact/to (e.g. "hari", "rahul") and do not invent contacts.`,
    `4a. MESSAGING SENDS: when the user provides exact message text, keep that text in message. When the user asks for a style/topic message, keep the topic/style so the send confirmation can draft it.`,
    `4b. FOLLOW-UP MESSAGES: when the user says things like "acknowledge this meeting", "notify them about it", or "send an update about this ticket", create the later messaging step as a follow-up to the earlier step and include a useful message body or placeholders referencing the earlier result.`,
