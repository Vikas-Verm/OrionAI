...
// ── Lesson generation ────────────────────────────────────────────────────
async function generateLessonForTopic({ goal, topic }) {
  if (!topic?.title) throw new Error("Topic title required");
  const context = describeGoalContext({ goal, topic });

  const userPrompt = [
    "Write a beginner-friendly lesson on this topic. Adapt to the topic type.",
    "Only include sections that genuinely help for this topic.",
    "",
    context,
    "",
    ...
    `{
      ...
      "explanation": "2 to 5 short paragraphs of plain-language explanation",
      "whyItMatters": "1 to 3 sentences on why this topic is useful",
      "keyPoints": ["short bullet", "..."],
      "examples": ["concrete example", "..."],
      "commonMistakes": ["mistake learners make", "..."],
      "revisionNotes": ["short revision-card-style note", "..."],
      "prerequisites": ["topic name or skill", "..."],
      "importantTerms": [{ "term": "string", "meaning": "1 sentence" }],
      "practicalUseCases": ["where this is used in real life", "..."]
    }`,
    "",
    "Rules:",
  ...
  return {
    ...
    whyItMatters:
      typeof parsed.whyItMatters === "string"
        ...
        : "",
    keyPoints: asStringArray(parsed.keyPoints, { max: 10 }),
    examples: asStringArray(parsed.examples, { max: 8 }),
    commonMistakes: asStringArray(parsed.commonMistakes, { max: 8 }),
    revisionNotes: asStringArray(parsed.revisionNotes, { max: 10 }),
    prerequisites: asStringArray(parsed.prerequisites, { max: 8 }),
    importantTerms: asTermArray(parsed.importantTerms, 12),
    practicalUseCases: asStringArray(parsed.practicalUseCases, { max: 8 }),
  };
}
...
async function answerTopicDoubt({ goal, topic, lesson, history = [], question }) {
  ...
  "followupSuggestion": "one short follow-up the learner might ask next, or empty string"
        ...
        "- Set usedAttachedMaterial to true only if you explicitly used the lesson context above.",
  ...
  return normalizeDoubtReply(raw, { hasLesson: !!lesson });
}
