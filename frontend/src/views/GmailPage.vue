<template>
    <div class="gm-root">
      ...
      <div class="gm-list-pane">
        ...
        <div class="gm-tab-row">
          ...
          <span class="gm-sync-badge" :class="`gm-sync-badge--${syncStatus === 'syncing_deep' ? 'syncing' : syncStatus}`"
            :title="syncStatus === 'error' ? (syncError || 'Sync error — check console') : ''">
            <span v-if="syncStatus === 'syncing' || syncStatus === 'syncing_deep' || syncStatus === 'connecting'" class="gm-sync-dot gm-sync-dot--spin"></span>
            <span v-else-if="syncStatus === 'error'" class="gm-sync-dot gm-sync-dot--error"></span>
            <span v-else class="gm-sync-dot gm-sync-dot--live"></span>
            {{ syncLabel() }}
          </span>
        </div>

        <!-- Deep sync progress banner -->
        <div v-if="syncStatus === 'syncing_deep'" class="gm-deep-banner">
          <span class="gm-sync-dot gm-sync-dot--spin" style="flex-shrink:0"></span>
          Fetching all emails… {{ deepSyncCount ? deepSyncCount.toLocaleString() + ' cached so far' : '' }}
        </div>

        <!-- Auth error banner -->
        <div v-if="syncStatus === 'error' && syncError && syncError.includes('scope')" class="gm-auth-banner">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          Gmail needs re-authorization. Please disconnect and reconnect Gmail in Integrations.
        </div>
        ...
        <div v-else class="gm-list-scroll" ref="emailListRef">
          <div v-if="!emails.length" class="gm-empty-list">
            <template v-if="syncStatus === 'syncing' || syncStatus === 'connecting'">
              <div class="gm-sync-spinner"></div>
              <p>Syncing your inbox…</p>
              <p class="gm-sync-sub">Fetching emails via IMAP, this happens once</p>
            </template>
            <template v-else>
        ...
        <template v-else>
          ...
          <div class="gm-thread-scroll">
            ...
            <div v-else class="gm-thread-msgs">
              <div v-for="(msg, idx) in threadMessages" :key="msg.id" class="gm-thread-msg">
                <!-- Message header -->
                <div class="gm-tm-hdr">
          ...
          <div class="gm-reply-dock">
            ...
            <div v-else class="gm-reply-composer">
              ...
              <div class="gm-rc-footer">
                ...
                <button class="gm-discard-btn" @click="showReply=false">Discard</button>
              </div>
            </div>
          </div>
        </template>
      ...
      <div v-if="showCompose" class="gm-compose-overlay" @click.self="showCompose=false">
        <div class="gm-compose-modal">
          ...
          <div class="gm-cm-footer">
            ...
            <button class="gm-cm-trash" @click="showCompose=false" title="Discard">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6M9 6V4h6v2"/></svg>
            </button>
          </div>
        </div>
  ...
  const storageQuota    = ref({ limit: 0, usage: 0, scopeError: false })
  // ── Sync state ──────────────────────────────────────────────────
  const syncStatus     = ref('connecting')
  const lastSyncAt     = ref(0)
  const syncError      = ref('')
  const requiresReauth = ref(false)
  const deepSyncCount  = ref(0)
  let   sseSource      = null
  let   observer       = null
  ...
  // ── Live Google storage quota ─────────────────────────────────────
  async function fetchStorageQuota() {
    try {
      const res = await api.get('/api/gmail/storage-quota')
      storageQuota.value = res.data || { limit: 0, usage: 0 }
    } catch {
      storageQuota.value = { limit: 0, usage: 0, scopeError: true }
    }
  }
...
//   }
  ...
  // ── SSE — real-time sync updates ───────────────────────────────
  function connectSSE() {
    if (sseSource) sseSource.close()
    sseSource = new EventSource('/api/gmail/events', { withCredentials: true })
    sseSource.addEventListener('sync_status', e => {
      try {
        const d = JSON.parse(e.data)
        syncStatus.value     = d.status
        lastSyncAt.value     = d.lastSyncAt || 0
        syncError.value      = d.error || ''
        requiresReauth.value = d.requiresReauth || false
        if (d.count) deepSyncCount.value = d.count
      } catch {}
    })
  ...
  function syncLabel() {
    ...
    if (syncStatus.value === 'syncing_deep') return 'Fetching all…'
  ...
  async function loadEmails(silent = false) {
    ...
    try {
      ...
      if (res.data.syncStatus) {
        syncStatus.value = res.data.syncStatus.status
        lastSyncAt.value = res.data.syncStatus.lastSyncAt || 0
      }
  ...
  async function sendReply() {
    ...
    try {
      await agentAPI.gmailReply({
        threadId: selectedEmail.value.threadId, messageId: selectedEmail.value.id,
        replyTo: selectedEmail.value.from, subject: selectedEmail.value.subject,
        body: replyDraft.value,
      })
      replySent.value = true; showReply.value = false; replyDraft.value = ''
  ...
  async function sendCompose() {
    ...
    try {
      await api.post('/api/gmail/send', { to: composeTo.value, subject: composeSubject.value, body: composeBody.value })
      showCompose.value = false; composeTo.value = ''; composeSubject.value = ''; composeBody.value = ''
    } catch (e) { console.error(e.message) }
  ...
  function displayFrom(email) {
    ...
    if (extractEmail(from).toLowerCase() === myEmail) {
      return email.origFrom || email.to || from
    }
    return from
  }
...
<\/script>
  ...
  .gm-sync-sub { font-size:11px; color:var(--text-muted); opacity:.55; margin-top:4px; }
  .gm-auth-banner {
    margin:0 10px 6px; padding:8px 10px; border-radius:7px;
    background:rgba(234,67,53,.12); border:1px solid rgba(234,67,53,.25);
    color:#EA4335; font-size:11.5px; display:flex; align-items:center; gap:6px; line-height:1.4;
  }
  .gm-deep-banner {
    margin:0 10px 6px; padding:7px 10px; border-radius:7px;
    background:rgba(66,133,244,.1); border:1px solid rgba(66,133,244,.2);
    color:#4285F4; font-size:11px; display:flex; align-items:center; gap:7px;
  }
